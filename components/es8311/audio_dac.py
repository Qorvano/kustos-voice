...
# Basis: ESPHome-es8311 (Upstream), geaendert ist ausschliesslich das
# Lautstaerke-Mapping des DAC: Regler linear in dB statt linear ueber den
# vollen Registerbereich (der bis +32 dB Digital-Gain reicht und oben hart
# uebersteuert). Details in NOTICE.md.
CODEOWNERS = ["@Qorvano"]
DEPENDENCIES = ["i2c"]
    ...
    cv.Schema(
        {
            ...
            # Gueltige Kombinationen aus MCLK-Vielfachem und Samplerate prueft
            # configure_clock_() zur Laufzeit gegen die Koeffizienten-Tabelle
            cv.Optional(CONF_MCLK_MULTIPLE, default=256): cv.int_range(min=1),
        }
    )
