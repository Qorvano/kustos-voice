...
bool ES7210::configure_mic_gain_() {
  const uint8_t user_gain = this->es7210_gain_reg_value_(this->mic_gain_);
  // Der Echo-Referenzkanal bekommt Line-Pegel vom Verstaerkerausgang; mit dem
  // Mikrofon-Gain wuerde er uebersteuern und die AEC-Schaetzung unbrauchbar
  // machen, daher fest 0 dB
  const uint8_t reference_gain = this->es7210_gain_reg_value_(0.0f);
  ...
  // Takt-Gates in REG01 je ADC-Paar (Referenztreiber esp-adf es7210_mic_select):
  // MIC1/2 loeschen Maske 0x0b, MIC3/4 Maske 0x15. Der Upstream-Treiber kennt
  // nur 2-Mic-Boards und loescht nur 0x0b - ADC3/4 blieben dadurch taktlos
  // und lieferten exakt Null in ihren TDM-Slots.
  static const uint8_t CLOCK_ON_MASK_ADC12 = 0x0b;
  static const uint8_t CLOCK_ON_MASK_ADC34 = 0x15;
  for (uint8_t mic = 0; mic < 4; ++mic) {
    const uint8_t gain_reg = ES7210_MIC1_GAIN_REG43 + mic;
    const uint8_t power_reg = (mic < 2) ? ES7210_MIC12_POWER_REG4B : ES7210_MIC34_POWER_REG4C;
    const uint8_t clock_mask = (mic < 2) ? CLOCK_ON_MASK_ADC12 : CLOCK_ON_MASK_ADC34;
    const uint8_t gain_value = (this->reference_channel_ == mic + 1) ? reference_gain : user_gain;
    ES7210_ERROR_CHECK(this->es7210_update_reg_bit_(ES7210_CLOCK_OFF_REG01, clock_mask, 0x00));
    ES7210_ERROR_CHECK(this->write_byte(power_reg, 0x00));
    ES7210_ERROR_CHECK(this->es7210_update_reg_bit_(gain_reg, 0x10, 0x10));
    ES7210_ERROR_CHECK(this->es7210_update_reg_bit_(gain_reg, 0x0f, gain_value));
  }
