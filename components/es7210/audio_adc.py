...
# Basis: ESPHome-es7210 (Upstream), erweitert um die TDM-Option (der Upstream-Code
# enthaelt den Registerpfad bereits, exponiert ihn aber nicht) und einen fest auf
# 0 dB gehaltenen Echo-Referenzkanal fuer AEC.
CODEOWNERS = ["@Qorvano"]
DEPENDENCIES = ["i2c"]
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_REFERENCE_CHANNEL, default=0): cv.int_range(min=0, max=4),
        }
    )
...
async def to_code(config):
    ...
    cg.add(var.set_enable_tdm(config[CONF_TDM]))
    cg.add(var.set_reference_channel(config[CONF_REFERENCE_CHANNEL]))
