...
esp_err_t I2SAudioSpeakerBase::borrow_i2s_channel_(size_t event_queue_size) {
  this->tx_handle_ = this->parent_->get_duplex_tx_handle();
  if (this->tx_handle_ == nullptr) {
    ESP_LOGE(TAG, "Duplex: Bus hat keinen TX-Kanal (Bus-Setup fehlgeschlagen?)");
    this->parent_->unlock();
...
void I2SAudioSpeakerBase::stop_i2s_driver_() {
  if (this->parent_->is_duplex()) {
    // Der Kanal gehoert dem Bus: nur anhalten (zurueck nach READY, Preload
    // funktioniert danach wieder), nicht loeschen. Kein gpio_reset - das
    // Pin-Routing gehoert dem Bus, und der RX-Taktmaster laeuft unberuehrt weiter.
    if (this->tx_handle_ != nullptr) {
