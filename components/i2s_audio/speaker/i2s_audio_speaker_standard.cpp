...
void I2SAudioSpeaker::run_speaker_task() {
  ...
  // Kompositions-Puffer: jede Iteration baut hier [Real-Audio][Stille-Pad]
  // zusammen und schreibt dann GENAU EINEN i2s_channel_write ueber die volle
  // Deskriptorgroesse. Mehrere Teil-Writes pro Deskriptor sind nicht sicher:
  // der IDF-Treiber wechselt bei Rueckstau (>= desc_num-2 fertige Deskriptoren
  // in der Queue) auch mitten im angebrochenen Deskriptor auf einen neuen
  // (i2s_common.c, uxQueueSpacesAvailable-Zweig); der Rest des angebrochenen
  // spielt dann Stille und alle folgenden Iterationen liegen versetzt ueber
  // Deskriptorgrenzen - hoerbar als Splice, und die audio_output_callback_-
  // Zeitstempel (AEC-Referenz-Ausrichtung!) stimmen dauerhaft nicht mehr.
  RAMAllocator<uint8_t> compose_allocator;
  uint8_t *compose_buffer = compose_allocator.allocate(dma_buffer_bytes);

  if (silence_buffer != nullptr && compose_buffer != nullptr) {
  ...
  if (!successful_setup) {
    ...
    while (this->pause_state_ || !this->timeout_.has_value() ||
      ...
      // Genau einen DMA-Puffer komponieren: so viel Real-Audio wie die Quelle
      // gerade hergibt (ggf. mehrere fill()-Aufrufe ueber einen Ringpuffer-
      // Wrap hinweg) in den Kompositions-Puffer kopieren, Rest mit Stille
      // fuellen - und dann in EINEM i2s_channel_write uebergeben. So beginnt
      // jeder Write treiberseitig exakt an einer Deskriptorgrenze und der
      // Fruehwechsel-Pfad des IDF (siehe Kommentar an compose_buffer) kann
      // keine Daten mehr verwaisen lassen.
      ...
      if (!this->pause_state_) {
        while (bytes_written_total < dma_buffer_bytes) {
          ...
          const size_t to_copy = std::min(audio_source->available(), dma_buffer_bytes - bytes_written_total);
          if (to_copy == 0) {
            // Ring buffer has nothing more to hand over right now; the remainder becomes silence.
            break;
          }
          memcpy(compose_buffer + bytes_written_total, audio_source->data(), to_copy);
          audio_source->consume(to_copy);
          bytes_written_total += to_copy;
          real_bytes_total += to_copy;
        }
      ...
      if (bytes_written_total < dma_buffer_bytes) {
        memset(compose_buffer + bytes_written_total, 0, dma_buffer_bytes - bytes_written_total);
      }

      size_t bw = 0;
      i2s_channel_write(this->tx_handle_, compose_buffer, dma_buffer_bytes, &bw, WRITE_TIMEOUT_TICKS);
      if (bw != dma_buffer_bytes) {
        // A short write breaks DMA descriptor alignment for every subsequent event;
        // the only safe recovery is to restart the task.
        ESP_LOGV(TAG, "Partial write: %u of %u bytes", (unsigned) bw, (unsigned) dma_buffer_bytes);
  ...
  if (compose_buffer != nullptr) {
    compose_allocator.deallocate(compose_buffer, dma_buffer_bytes);
...
esp_err_t I2SAudioSpeaker::start_i2s_driver(audio::AudioStreamInfo &audio_stream_info) {
  ...
  if (this->parent_->is_duplex()) {
    // Duplex-TDM-Bus: der TX-Kanal existiert bereits mit fester Taktung; der
    // Stream muss exakt passen, eine Rekonfiguration ist nicht moeglich
    if (audio_stream_info.get_sample_rate() != this->parent_->get_duplex_sample_rate() ||
        audio_stream_info.get_bits_per_sample() != this->parent_->get_duplex_slot_bits()) {
      ESP_LOGE(TAG, "Duplex: Stream %" PRIu32 " Hz/%u bit passt nicht zur Bus-Taktung %" PRIu32 " Hz/%u bit",
               audio_stream_info.get_sample_rate(), (unsigned) audio_stream_info.get_bits_per_sample(),
               this->parent_->get_duplex_sample_rate(), (unsigned) this->parent_->get_duplex_slot_bits());
      this->parent_->unlock();
      return ESP_ERR_NOT_SUPPORTED;
    }
    return this->borrow_i2s_channel_(I2S_EVENT_QUEUE_COUNT);
  }
