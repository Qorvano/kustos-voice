...
# Vollduplex-TDM: der Bus besitzt TX+RX eines Controllers (ein
# i2s_new_channel-Aufruf, RX-first = Clock-Master beim dauerlaufenden Mikrofon)
CONF_DUPLEX = "duplex"
CONF_SLOT_BIT_WIDTH = "slot_bit_width"
CONF_TOTAL_SLOTS = "total_slots"
CONF_TX_SLOTS = "tx_slots"
CONF_RX_SLOTS = "rx_slots"
CONF_SLOT_MONITOR = "slot_monitor"
CONF_MONO = "mono"
CONF_LEFT = "left"
CONF_RIGHT = "right"
CONF_STEREO = "stereo"
...
def _validate_duplex(config):
    total = config[CONF_TOTAL_SLOTS]
    for key in (CONF_TX_SLOTS, CONF_RX_SLOTS):
        for slot in config[key]:
            if slot >= total:
                raise cv.Invalid(f"{key}: Slot {slot} liegt ausserhalb von total_slots={total}")
    return config


DUPLEX_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.Required(CONF_I2S_DOUT_PIN): pins.internal_gpio_output_pin_number,
            cv.Required(CONF_I2S_DIN_PIN): pins.internal_gpio_input_pin_number,
            cv.Optional(CONF_SAMPLE_RATE, default=48000): cv.int_range(min=8000),
            cv.Optional(CONF_SLOT_BIT_WIDTH, default=16): cv.one_of(16, 32, int=True),
            cv.Optional(CONF_TOTAL_SLOTS, default=4): cv.int_range(min=2, max=16),
            cv.Optional(CONF_TX_SLOTS, default=[0, 2]): cv.ensure_list(
                cv.int_range(min=0, max=15)
            ),
            cv.Optional(CONF_RX_SLOTS, default=[0, 1, 2, 3]): cv.ensure_list(
                cv.int_range(min=0, max=15)
            ),
            cv.Optional(CONF_USE_APLL, default=True): cv.boolean,
            cv.Optional(CONF_MCLK_MULTIPLE, default=256): cv.one_of(*I2S_MCLK_MULTIPLE),
            # Bring-up-Diagnose: sekuendliches RMS-Log je Slot; exklusiv,
            # nicht zusammen mit einem RX-Konsumenten verwenden
            cv.Optional(CONF_SLOT_MONITOR, default=False): cv.boolean,
        }
    ),
    _validate_duplex,
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_DUPLEX): DUPLEX_SCHEMA,
    },
)
...
async def to_code(config):
    ...
    if duplex := config.get(CONF_DUPLEX):
        cg.add(var.set_duplex_enabled(True))
        cg.add(var.set_duplex_dout_pin(duplex[CONF_I2S_DOUT_PIN]))
        cg.add(var.set_duplex_din_pin(duplex[CONF_I2S_DIN_PIN]))
        cg.add(var.set_duplex_sample_rate(duplex[CONF_SAMPLE_RATE]))
        cg.add(var.set_duplex_slot_bits(duplex[CONF_SLOT_BIT_WIDTH]))
        cg.add(var.set_duplex_total_slots(duplex[CONF_TOTAL_SLOTS]))
        cg.add(var.set_duplex_tx_slot_mask(sum(1 << s for s in duplex[CONF_TX_SLOTS])))
        cg.add(var.set_duplex_rx_slot_mask(sum(1 << s for s in duplex[CONF_RX_SLOTS])))
        cg.add(var.set_duplex_use_apll(duplex[CONF_USE_APLL]))
        cg.add(var.set_duplex_mclk_multiple(I2S_MCLK_MULTIPLE[duplex[CONF_MCLK_MULTIPLE]]))
        cg.add(var.set_duplex_slot_monitor(duplex[CONF_SLOT_MONITOR]))
